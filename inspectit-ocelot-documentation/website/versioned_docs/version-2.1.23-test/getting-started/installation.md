---
id: version-2.1.23-test-installation
title: Installation
...
```bash
$ java -javaagent:"/path/to/inspectit-ocelot-agent-2.1.23-test.jar" -jar my-java-program.jar
```
...
```bash
$ java -jar inspectit-ocelot-agent-2.1.23-test.jar <PID> [<AGENT_ARGUMENTS>]
```
...
```bash
$ java -jar inspectit-ocelot-agent-2.1.23-test.jar 1337 '{"inspectit":{"service-name":"my-agent"}}'
```
...
```bash
$ ./jattach.sh <PID> load instrument false /path/to/inspectit-ocelot-agent-2.1.23-test.jar='{"inspectit.service-name" : "MyService"}'
```

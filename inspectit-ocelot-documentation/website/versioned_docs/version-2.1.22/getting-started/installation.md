---
id: version-2.1.22-installation
title: Installation
...
```bash
$ java -javaagent:"/path/to/inspectit-ocelot-agent-2.1.22.jar" -jar my-java-program.jar
```
...
```bash
$ java -jar inspectit-ocelot-agent-2.1.22.jar <PID> [<AGENT_ARGUMENTS>]
```
...
```bash
$ java -jar inspectit-ocelot-agent-2.1.22.jar 1337 '{"inspectit":{"service-name":"my-agent"}}'
```
...
```bash
$ ./jattach.sh <PID> load instrument false /path/to/inspectit-ocelot-agent-2.1.22.jar='{"inspectit.service-name" : "MyService"}'
```

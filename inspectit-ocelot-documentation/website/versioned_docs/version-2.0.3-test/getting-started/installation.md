---
id: version-2.0.3-test-installation
title: Installation
...
```bash
$ java -javaagent:"/path/to/inspectit-ocelot-agent-2.0.3-test.jar" -jar my-java-program.jar
```
...
```bash
$ java -jar inspectit-ocelot-agent-2.0.3-test.jar <PID> [<AGENT_ARGUMENTS>]
```
...
```bash
$ java -jar inspectit-ocelot-agent-2.0.3-test.jar 1337 '{"inspectit":{"service-name":"my-agent"}}'
```
...
```bash
$ ./jattach.sh <PID> load instrument false /path/to/inspectit-ocelot-agent-2.0.3-test.jar='{"inspectit.service-name" : "MyService"}'
```
